...
for (const packageName of ['@viktar-b/cso-core', '@viktar-b/cso-react']) {
  run(repoRoot, 'npm', [
    'pack',
    '--workspace',
    packageName,
...
const coreArchive = packedArchive('viktar-b-cso-core-');
const reactArchive = packedArchive('viktar-b-cso-react-');
...
assert.throws(() => require.resolve('@viktar-b/cso-core/src/index.ts'), 'Core private source is not exported');
...
import { CalculationSourceObjectSchema, createSheetFromCalculationSourceObject } from '@viktar-b/cso-core';
import * as esm from '@viktar-b/cso-react';
const require = createRequire(import.meta.url);
const cjs = require('@viktar-b/cso-react');
...
const css = readFileSync(require.resolve('@viktar-b/cso-react/style.css'), 'utf8');
assert(css.includes('formula-sheet-printing'), 'Packaged CSS contains print rules');
assert.throws(() => require.resolve('@viktar-b/cso-react/src/FormulaSheet.tsx'), 'React private source is not exported');
...
import { CalculationSourceObjectSchema, createSheetFromCalculationSourceObject, type SheetDocument } from '@viktar-b/cso-core';
import { FormulaSheet, AsciiMathView } from '@viktar-b/cso-react';
...
const nodeConsumer = `
import { verifyExecution, type VerifyExecution, type VerifyExecutionInput, type VerificationReport } from '@viktar-b/cso-core';
...
write(
  ...
  'contract-browser.ts',
  `
import { BoundPreparedDocumentSchema, ExecutionResponseSchema, PreparedDocumentSchema, VerificationReportSchema, sourceClosureHash, verifyExecution } from '@viktar-b/cso-core';
